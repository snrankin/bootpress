...
function togglerButton($target, $args = array())
{
    ...
    $label = itemWrapperHTML($label, array('tag' => 'span', 'class' => $label_classes));

    $icon = itemWrapperHTML($icon, array('tag' => 'span', 'class' => array('navbar-toggler-icon')));

    $output = itemWrapperHTML($icon . $label, $atts);

    return $output;
}

function bootpressNav($menu, $dropdown, $args = array())
{
    ...
    $defaults = array(
        'menu'            => $menu,
        'container'       => 'div',
        'container_id'    => $menu->slug . '-container',
    ...
    if (!isset($options['menu_id'])) {
        $options['menu_id'] = $menu->slug . '-' . mt_rand();
    }
    if (!isset($options['container_id'])) {
        $options['container_id'] = $options['menu_id'] . '-container';
    }
    if ($dropdown == true) {
        $item_content .= togglerButton($options['container_id']);
        $options['container_class'] = outputClasses($options['container_class'], array('collapse', 'navbar-collapse'));
    }
    if (is_array($options['container_class'])) {
        $options['container_class'] = implode(' ', $options['container_class']);
    }
    $item_content .= wp_nav_menu($options);
