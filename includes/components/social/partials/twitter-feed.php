...
function returnTweet($screen_name, $number_of_tweets, $add_links, $add_username, $add_author_link)
{
    ...
    foreach ($items as $item) {
        ...
        $tweetIcon      = '<i class="amerisleep-tweet-icon"></i>';
        $tweetAuthorTxt = '@' . $item['user']['screen_name'];
        $tweetAuthor    = ($add_author_link == 1) ? '<a href="https://twitter.com/' . $tweetAuthorTxt . '" target="_blank" class="amerisleep-tweet-meta-txt">' . $tweetAuthorTxt . '</a>' : '<span class="amerisleep-tweet-meta-txt">' . $tweetAuthorTxt . '</span>';

        $tweets .= '<div class="amerisleep-tweet">';
        $tweets .= '<p class="amerisleep-tweet-txt">' . $tweetTxt . '</p>';
        $tweets .= ($add_username == 1) ? '<p class="amerisleep-tweet-meta amerisleep-tweet-author"><span class="amerisleep-tweet-meta-inner">' . $tweetIcon . $tweetAuthor . '</span></p>' : '';
        $tweets .= '<p class="amerisleep-tweet-date amerisleep-tweet-meta"><span class="amerisleep-tweet-meta-inner">';
        $tweets .= $tweetIcon;
        $tweets .= '<span class="amerisleep-tweet-meta-txt">';
        $tweets .= $tweetDate;
        $tweets .= '</span>';
        $tweets .= '</span></p>';
        $tweets .= '</div>';
